 ...
 */
public class SlideshowController extends Application
{
	...
	private void clickedBackwardArrow(ActionEvent event)
	{
		...
		if(position==-1)
			position = selectedAlbum.photos.size()-1; //loop to back
		
		selectedAlbum = UserHomeController.getSelectedAlbum();
		selectedPhoto = selectedAlbum.photos.get(position); 
	...
	private void clickedForwardArrow(ActionEvent event)
	{
		...
		if(position == selectedAlbum.photos.size())
