 ...
 * @author Karl Xu
 *
 * AddTagController to broker between front-end and back-end for add tag
 */
public class AddTagController extends Application
{
	/**
	...
	/**
	 * clickedExampleTags is the method called when user clicks the example tags link
	 * @param event
	 */
	@FXML
	private void clickedExampleTags(ActionEvent event)
	{
		//System.out.println("clicked example tags"); //REMOVE
		
		//go to example tags [STORYBOARD - 11]
		goToExampleTags(event);
	}
	...
	private void clickedDone(ActionEvent event)
	{
		...
		//CALL KARL'S METHOD TO ADD TAG
		selectedPhoto.addTag(tagType,tagValueStr);
	...
	/**
	 * goToExampleTags is the method to go to the example tags UI
	 * @param event is the action event
	 */
	private void goToExampleTags(Event event)
	{
		try {
			
			FXMLLoader loader = new FXMLLoader();
			loader.setLocation(getClass().getResource("/view/ExampleTags.fxml"));
			AnchorPane root = (AnchorPane)loader.load();
			
			Node source = (Node) event.getSource();
			Stage stage = (Stage) source.getScene().getWindow();
			
			ExampleTagsController exampleTagsController = (ExampleTagsController)loader.getController();
			exampleTagsController.start(stage);
			
			Scene scene = new Scene(root, 1000, 600);
			
			stage.setScene(scene);
			stage.setTitle("Example Tags");
			stage.setResizable(false);
			stage.show();
		} catch(Exception e) {
			e.printStackTrace();
